...
namespace Linq
{
    ...
    public static class JoinOperations
    {
        ...
        public static IEnumerable<(string category, string productName)> JoinQuery()
        {
            ...
            return categories.Join(products,category => category,product => product.Category,
      (category, product) => (category, productName: product.ProductName));
        ...
        public static IEnumerable<(string category, IEnumerable<Product> productsName)> GroupJoinQuery()
        {
            ...
            return categories.GroupJoin(products, category =>category, product=> product.Category, (category, matchingproducts) => (category, matchingproducts));
        }
        ...
        public static IEnumerable<(string category, string productName)> LeftOuterJoin()
        {
            ...
            return categories
        .GroupJoin(products,category => category, product => product.Category,(category, matchingProducts) => matchingProducts.Any()? matchingProducts.Select(product => (category, product.ProductName))
                    : new[] { (category, "(No products)") })
