...
namespace Linq
{
    ...
    public static class AggregationOperations
    {
        ...
        public static IEnumerable<(string customerId, int orderCount)> CustomersOrdersCount()
        {
            ...
            return customers.Select(c=> (customerID: c.CustomerId, orderCount: c.Orders.Count()));
        }
        ...
        public static IEnumerable<(string category, int productCount)> ProductsInCategoryCount()
        {
            ...
            return products.GroupBy(p => p.Category).Select(gr => (category: gr.Key, productCount: gr.Count()));
        ...
        {
            List<Product> products = Products.ProductList;
            return products.GroupBy(p =>  p.Category).Select(gr => (category: gr.Key, totalUnitsInStock: gr.Sum(p=>p.UnitsInStock)));
        ...
        public static int MinByLength()
        {
            ...
            return words.Select(w => w.Length).Min();
        }
        ...
        public static IEnumerable<(string category, decimal cheapestPrice)> GetCheapestPrice()
        {
            ...
            return products.GroupBy(p => p.Category)
                     .Select(gr => (gr.Key, gr.Min(p => p.UnitPrice)));
        }
        ...
        {
            string[] words = { "cherry", "apple", "blueberry" };
            return words.Select(w => w.Length).Max();
        }
        ...
        public static IEnumerable<(string category, decimal mostExpensivePrice)> GetMostExpensivePrice()
        {
            ...
            return products.GroupBy(p => p.Category)
                    .Select(gr => (gr.Key, gr.Max(p => p.UnitPrice)));
        }
        ...
        public static double AverageByLength()
        {
            ...
            return words.Select(w => w.Length).Average();
        }
        ...
        public static IEnumerable<(string Category, decimal averagePrice)> AveragePrice()
        {
            ...
            return products.GroupBy(p => p.Category)
                     .Select(gr => (gr.Key, gr.Average(p => p.UnitPrice)));
        }
        ...
        public static double Aggregate()
        {
            ...
            return doubles.Aggregate(1.0, (acc, x) =>acc*x);
        }
